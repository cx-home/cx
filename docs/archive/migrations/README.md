...
| --------- | ----- | ---------- |
| v0.6.0 → v0.7.0 | [v0.6-to-v0.7.md](v0.6-to-v0.7.md) | Full XQuery 4.0 / XPath 4.0 parity (ADR 0022); CXPath axes; operator-token surface; ~80+ fn library; map/array runtime values; streaming evaluator; `cx:lang` resolution; HTMX example pattern; Parquet bridges in Py/Go/Rust; C ABI rename (`cx_eval_cxl*` → `cx_eval*`); 9 → 5 active bindings |
