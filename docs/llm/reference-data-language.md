# Reference: the CX data language — v0.16.0
...
Bare tokens are typed on read. `true`/`false` are booleans, integer and
float literals are numbers, `null` is the explicit null, and anything else
is an atom. Quoting suppresses all of it and gives you a string.
