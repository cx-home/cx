# CX primer for LLM assistants — v0.17.0
...
> every output was re-recorded by running the `cx` v0.17.0 binary at
...
5. **Scalars are typed on read.** `true` is a boolean, `42` an int, `1.5` a
   **decimal** (*not* a float — see §2), `null` the explicit null, a bare word
   an atom, quoted text a string. Quoting is how you *stop* auto-typing.
...
8. **…but an error may not leave silently.** An externalizing effect — a
   store document write, an HTTP response — refuses to carry a document
   containing an `[err]` at any depth out of the program. You permit it
   explicitly with `errs=:permit` or not at all. See §6.
9. **A sequence value is never element content.** `[violations $vs]` where
   `$vs` is a sequence is *refused*, not spliced. The idiom is
   `[violations [?splice $vs]]`. Nothing auto-splices in CX. See §5.
...
12. **`cx [flags] FILE [program-args…]`.** Flags bind *before* the file.
...
### `1.5` is a decimal, not a float

Worth stopping on, because almost every language you know reads a bare
fraction as a binary float and CX does not.

| Kind | How you get one |
|---|---|
| **decimal** | a plain fraction literal: `1.5`, `2.50`, `0.1`. Exact, base-10, arbitrary precision |
| **float** | an `e`-exponent literal (`1.5e0`), a `::float` annotation, `[cast x :float]`, division `[/ …]`, `[$avg …]`, and the float-returning `math` functions |

`[price 19.99]` is a decimal and stays exact; `[/ 7 2]` is a float.
Arithmetic keeps a decimal a decimal — `[* 2 1.5]` is `3.0`, and ints mix in
freely — but the two kinds never mix with each other:

#### Mixing a decimal literal with a float in arithmetic

Almost every language reads a bare fraction as a binary float. CX does not:
`1.5` is a **decimal** — exact, base-10, arbitrary precision — and only an
`e`-exponent literal (`1.5e0`), a `::float` annotation, a `[cast … :float]`,
a division, `[$avg]`, or a float-returning `math` function produces a float.

The two kinds do not silently mix. Adding them is a loud refusal naming the
only bridge, rather than a quiet promotion that would put binary rounding
into a money column — which is precisely the class this rule exists to close.
...
`prog.cx`
```cx
[?let [= $subtotal 19.99] [= $shipping 5.01] [total [+ $subtotal $shipping]]]
```
...
<sub>Fixtures: `ap-decimal-float-mix-wrong` / `ap-decimal-float-mix-right` in `conformance/llm/antipatterns.cxd`</sub>

The formatters keep decimals exactly as written:

`prog.cx`
```cx
[?lib 'cx-stdlib/format']
[$format:pretty {a: 1, b: 1.5, c: :ops, d: "s"}]
```
...
The kinds are also distinct to **identity and to equality**: `1.5` and `1.5e0`
denote the same number but are different values, so they canonicalize
differently, hash differently, and `[$eq]` answers `false`. (Contrast §6's
decimal-vs-*string* case, which is `true` — atomization crosses the
scalar/text line, never the decimal/float one.) `reference-data-language.md`
has the image rule.
...
### Build element content with `[?splice]`, never by dropping a sequence in

This is the single rule most likely to break the first program you write,
because every templating language you have seen splices a list into a slot
silently. **CX does not.** A non-empty sequence *value* in element-content
position is refused, and the diagnostic names the idiom you meant:
...
* **`[?for]` and `[?splice]` contribute siblings; everything else contributes
  one child.** A binding read, a call result, an `[?if]` / `[?let]` /
  `[?match]` result each contribute exactly one — so one of those yielding a
  sequence refuses. Conditional multi-contribution is `[?splice [?if …]]`.
* **An argument is not content.** A `$`-head call, an operator head, or a
  closure call *consumes* a sequence argument happily. Only plain element
  construction refuses.
* **Absence contributes nothing.** `[c $u/nothing]` builds `[c]`, not an
  element with an empty child.
* **`[break]` / `[continue]` are exempt.** They are `[?loop]`'s positional
  value carriers, not documents: absence rides as the empty sequence and a
  sequence rides whole, because the loop rebinds positionally.

`reference-code-language.md` has the worked example for each.
...
Everything above is about errors *inside* a program, where they are ordinary
values. Leaving is different. An **externalizing effect** refuses to carry a
document containing an `[err]` at any depth out of the program:
...
`cx-err:CXER0275` names the first contained err's code and its path. Two
families are guarded, and they are the two that turn a refusal into someone
else's data:

* **Store document writes** — `put-doc`, `put-doc-stream`, `put-doc-text`
  (it stores a *parsed* document), and `modify-doc` (its action payload is
  the injection vehicle).
* **HTTP response emission** — a handler result, whether a `[response]`
  envelope, a bare document, or a bare `[err]`. The refusal is a loud 500;
  the bare-err-as-200 shape is exactly the silent class this closes.

Externalizing a refusal *deliberately* is legal, and it has **one spelling
everywhere**: `errs=:permit` — a key in the trailing opts map on the store
verbs, an attribute on `[response]` / `[sse-subscribe]`:
...
Exempt by construction: blob and byte writes (bytes carry no `[err]` to
find), channels, bindings, matching, formatting, the journal (an error event
is exactly what a journal is for), fabric emit, and the run-surface print. An
err in a collection is still just a value:
...
that reports success while its output carries the refusals as data.

### Two equality notions, both deliberate

`[$eq]` / `[=]` is **value** equality under the atomization policy: a scalar
compares against the string of its image, so these are `true` by design.
...
`cx canonical` / `cx hash` / `cx eq` / a store handle are the **identity**
notion — type-faithful over the strict-canonical bytes — and they distinguish
exactly those pairs. When you are asking "is this the same document" for a
cache key or a content address, you want identity, not `[$eq]`.
...
6. Every module you imported is in the §8 catalog. You did not invent one.
7. Every effect has its `--allow-…` grant, and you granted the narrowest one.
8. Strings that must stay text are quoted.
9. No sequence value sits in element content. Where you meant many children,
   you wrote `[?splice …]` or `[?for …]`.
10. Every externalizing effect either carries no `[err]` or says
...
Load one only when the task needs it — this primer is meant to be the whole
context for ordinary work.
...
| `playbook-xap.md` | **you are building a feature deployment** — composing feature grammars, authoring the `*.xap.cxd` deployment document, bootstrapping identity, hosting, or projecting a ux-web surface |
