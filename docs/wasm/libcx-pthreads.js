...
// include: scripts/wasm/jspi_mode_prejs.js
// jspi_mode_prejs.js — stamped into the emscripten factory via --pre-js
// when the build runs with ASYNCIFY=1 ASYNCIFY_MODE=2 (JSPI, -sASYNCIFY=2).
// cxlib.js reads Module.cxAsyncifyMode to pick the async call path:
//   2         → JSPI: wrap raw wasm exports with WebAssembly.promising
//               (ccall {async:true} is classic-Asyncify plumbing and
//               throws `TypeError: ret.then is not a function` under
//               JSPI — measured, emcc 5.0.7).
//   undefined → classic Asyncify (or plain build): keep the historic
//               ccall {async:true} path. Old artifacts never define
//               this flag, so they keep working unchanged.
// This must be a build-time stamp, not a runtime probe: a classic
// -sASYNCIFY=1 artifact running in a JSPI-capable browser would pass
// any `WebAssembly.Suspending` feature test, and wrapping a
// classic-instrumented export with WebAssembly.promising silently
// bypasses the Asyncify unwind machinery.
Module["cxAsyncifyMode"] = 2;

// end include: scripts/wasm/jspi_mode_prejs.js
var arguments_ = [];
...
function findWasmBinary() {
  return locateFile("libcx-pthreads.wasm");
}
...
function ___syscall_pipe2(fdPtr, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, fdPtr, flags);
  try {
    if (fdPtr == 0) {
...
var ___syscall_poll = function(fds, nfds, timeout) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 2, fds, nfds, timeout);
  let innerFunc = () => {
    try {
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 0, 1, fd, message, length, flags, addr, addr_len);
  try {
    var sock = getSocketFromFD(fd);
    if (!addr) {
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, len, prot, flags, fd, offset, allocated, addr);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, addr, len, prot, flags, fd, offset);
  offset = bigintToI53Checked(offset);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    if (prot & 2) {
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 0, 1, fd, pbuf);
  try {
    var rightsBase = 0;
...
function _getaddrinfo(node, service, hint, out) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(44, 0, 1, node, service, hint, out);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_accept4, ___syscall_bind, ___syscall_chdir, ___syscall_chmod, ___syscall_connect, ___syscall_dup, ___syscall_dup3, ___syscall_faccessat, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_ftruncate64, ___syscall_getcwd, ___syscall_getdents64, ___syscall_getsockname, ___syscall_ioctl, ___syscall_listen, ___syscall_lstat64, ___syscall_mkdirat, ___syscall_newfstatat, ___syscall_openat, ___syscall_pipe2, ___syscall_poll, ___syscall_readlinkat, ___syscall_recvfrom, ___syscall_renameat, ___syscall_rmdir, ___syscall_sendto, ___syscall_socket, ___syscall_stat64, ___syscall_symlinkat, ___syscall_unlinkat, __mmap_js, __munmap_js, _environ_get, _environ_sizes_get, _fd_close, _fd_fdstat_get, _fd_read, _fd_seek, _fd_sync, _fd_write, _getaddrinfo ];
...
var _cx_free = Module["_cx_free"] = makeInvalidEarlyAccess("_cx_free");

var _cx_wasm_set_arena_size = Module["_cx_wasm_set_arena_size"] = makeInvalidEarlyAccess("_cx_wasm_set_arena_size");

var _cx_wasm_reset = Module["_cx_wasm_reset"] = makeInvalidEarlyAccess("_cx_wasm_reset");

var _cx_version = Module["_cx_version"] = makeInvalidEarlyAccess("_cx_version");

var _cx_features = Module["_cx_features"] = makeInvalidEarlyAccess("_cx_features");

var _cx_to_cx_compact = Module["_cx_to_cx_compact"] = makeInvalidEarlyAccess("_cx_to_cx_compact");

var _cx_to_cx = Module["_cx_to_cx"] = makeInvalidEarlyAccess("_cx_to_cx");

var _cx_to_xml = Module["_cx_to_xml"] = makeInvalidEarlyAccess("_cx_to_xml");

var _cx_to_json = Module["_cx_to_json"] = makeInvalidEarlyAccess("_cx_to_json");

var _cx_to_yaml = Module["_cx_to_yaml"] = makeInvalidEarlyAccess("_cx_to_yaml");

var _cx_to_toml = Module["_cx_to_toml"] = makeInvalidEarlyAccess("_cx_to_toml");

var _cx_to_ast_bin = Module["_cx_to_ast_bin"] = makeInvalidEarlyAccess("_cx_to_ast_bin");

var _cx_canonical = Module["_cx_canonical"] = makeInvalidEarlyAccess("_cx_canonical");

var _cx_hash = Module["_cx_hash"] = makeInvalidEarlyAccess("_cx_hash");

var _cx_code_tree = Module["_cx_code_tree"] = makeInvalidEarlyAccess("_cx_code_tree");

var _cx_code_eval = Module["_cx_code_eval"] = makeInvalidEarlyAccess("_cx_code_eval");

var _cx_code_eval_with_len = Module["_cx_code_eval_with_len"] = makeInvalidEarlyAccess("_cx_code_eval_with_len");

var _cx_code_diagram = Module["_cx_code_diagram"] = makeInvalidEarlyAccess("_cx_code_diagram");

var _cx_code_diagram_with_level = Module["_cx_code_diagram_with_level"] = makeInvalidEarlyAccess("_cx_code_diagram_with_level");

var _cx_code_eval_streaming = Module["_cx_code_eval_streaming"] = makeInvalidEarlyAccess("_cx_code_eval_streaming");

var _cx_code_ast_json = Module["_cx_code_ast_json"] = makeInvalidEarlyAccess("_cx_code_ast_json");

var _cx_wasm_set_wall_sleep = Module["_cx_wasm_set_wall_sleep"] = makeInvalidEarlyAccess("_cx_wasm_set_wall_sleep");

var _cx_wasm_is_asyncify = Module["_cx_wasm_is_asyncify"] = makeInvalidEarlyAccess("_cx_wasm_is_asyncify");

var __vinit = Module["__vinit"] = makeInvalidEarlyAccess("__vinit");

var __vcleanup = Module["__vcleanup"] = makeInvalidEarlyAccess("__vcleanup");
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports["cx_free"] != "undefined", "missing Wasm export: cx_free");
  assert(typeof wasmExports["cx_wasm_set_arena_size"] != "undefined", "missing Wasm export: cx_wasm_set_arena_size");
  assert(typeof wasmExports["cx_wasm_reset"] != "undefined", "missing Wasm export: cx_wasm_reset");
  assert(typeof wasmExports["cx_version"] != "undefined", "missing Wasm export: cx_version");
  assert(typeof wasmExports["cx_features"] != "undefined", "missing Wasm export: cx_features");
  assert(typeof wasmExports["cx_to_cx_compact"] != "undefined", "missing Wasm export: cx_to_cx_compact");
  assert(typeof wasmExports["cx_to_cx"] != "undefined", "missing Wasm export: cx_to_cx");
  assert(typeof wasmExports["cx_to_xml"] != "undefined", "missing Wasm export: cx_to_xml");
  assert(typeof wasmExports["cx_to_json"] != "undefined", "missing Wasm export: cx_to_json");
  assert(typeof wasmExports["cx_to_yaml"] != "undefined", "missing Wasm export: cx_to_yaml");
  assert(typeof wasmExports["cx_to_toml"] != "undefined", "missing Wasm export: cx_to_toml");
  assert(typeof wasmExports["cx_to_ast_bin"] != "undefined", "missing Wasm export: cx_to_ast_bin");
  assert(typeof wasmExports["cx_canonical"] != "undefined", "missing Wasm export: cx_canonical");
  assert(typeof wasmExports["cx_hash"] != "undefined", "missing Wasm export: cx_hash");
  assert(typeof wasmExports["cx_code_tree"] != "undefined", "missing Wasm export: cx_code_tree");
  assert(typeof wasmExports["cx_code_eval"] != "undefined", "missing Wasm export: cx_code_eval");
  assert(typeof wasmExports["cx_code_eval_with_len"] != "undefined", "missing Wasm export: cx_code_eval_with_len");
  assert(typeof wasmExports["cx_code_diagram"] != "undefined", "missing Wasm export: cx_code_diagram");
  assert(typeof wasmExports["cx_code_diagram_with_level"] != "undefined", "missing Wasm export: cx_code_diagram_with_level");
  assert(typeof wasmExports["cx_code_eval_streaming"] != "undefined", "missing Wasm export: cx_code_eval_streaming");
  assert(typeof wasmExports["cx_code_ast_json"] != "undefined", "missing Wasm export: cx_code_ast_json");
  assert(typeof wasmExports["cx_wasm_set_wall_sleep"] != "undefined", "missing Wasm export: cx_wasm_set_wall_sleep");
  assert(typeof wasmExports["cx_wasm_is_asyncify"] != "undefined", "missing Wasm export: cx_wasm_is_asyncify");
  assert(typeof wasmExports["_vinit"] != "undefined", "missing Wasm export: _vinit");
  assert(typeof wasmExports["_vcleanup"] != "undefined", "missing Wasm export: _vcleanup");
  ...
  _cx_free = Module["_cx_free"] = createExportWrapper("cx_free", 1);
  _cx_wasm_set_arena_size = Module["_cx_wasm_set_arena_size"] = createExportWrapper("cx_wasm_set_arena_size", 1);
  _cx_wasm_reset = Module["_cx_wasm_reset"] = createExportWrapper("cx_wasm_reset", 0);
  _cx_version = Module["_cx_version"] = createExportWrapper("cx_version", 0);
  _cx_features = Module["_cx_features"] = createExportWrapper("cx_features", 0);
  _cx_to_cx_compact = Module["_cx_to_cx_compact"] = createExportWrapper("cx_to_cx_compact", 2);
  _cx_to_cx = Module["_cx_to_cx"] = createExportWrapper("cx_to_cx", 2);
  _cx_to_xml = Module["_cx_to_xml"] = createExportWrapper("cx_to_xml", 2);
  _cx_to_json = Module["_cx_to_json"] = createExportWrapper("cx_to_json", 2);
  _cx_to_yaml = Module["_cx_to_yaml"] = createExportWrapper("cx_to_yaml", 2);
  _cx_to_toml = Module["_cx_to_toml"] = createExportWrapper("cx_to_toml", 2);
  _cx_to_ast_bin = Module["_cx_to_ast_bin"] = createExportWrapper("cx_to_ast_bin", 2);
  _cx_canonical = Module["_cx_canonical"] = createExportWrapper("cx_canonical", 2);
  _cx_hash = Module["_cx_hash"] = createExportWrapper("cx_hash", 2);
  _cx_code_tree = Module["_cx_code_tree"] = createExportWrapper("cx_code_tree", 3);
  _cx_code_eval = Module["_cx_code_eval"] = createExportWrapper("cx_code_eval", 4);
  _cx_code_eval_with_len = Module["_cx_code_eval_with_len"] = createExportWrapper("cx_code_eval_with_len", 6);
  _cx_code_diagram = Module["_cx_code_diagram"] = createExportWrapper("cx_code_diagram", 4);
  _cx_code_diagram_with_level = Module["_cx_code_diagram_with_level"] = createExportWrapper("cx_code_diagram_with_level", 3);
  _cx_code_eval_streaming = Module["_cx_code_eval_streaming"] = createExportWrapper("cx_code_eval_streaming", 8);
  _cx_code_ast_json = Module["_cx_code_ast_json"] = createExportWrapper("cx_code_ast_json", 2);
  _cx_wasm_set_wall_sleep = Module["_cx_wasm_set_wall_sleep"] = createExportWrapper("cx_wasm_set_wall_sleep", 1);
  _cx_wasm_is_asyncify = Module["_cx_wasm_is_asyncify"] = createExportWrapper("cx_wasm_is_asyncify", 0);
  __vinit = Module["__vinit"] = createExportWrapper("_vinit", 2);
  __vcleanup = Module["__vcleanup"] = createExportWrapper("_vcleanup", 0);
...
if (typeof exports === 'object' && typeof module === 'object') {
  module.exports = createCxModule;
  // This default export looks redundant, but it allows TS to import this
  // commonjs style module.
  module.exports.default = createCxModule;
} else if (typeof define === 'function' && define['amd'])
  define([], () => createCxModule);
