# `$redis-*` — external Redis engine (catalogue entry)
...
Catalogue entry for the `redis_stdlib_builtin` dispatch family. The normative
...
there. This file exists so the stdlib catalogue (and its gate) carries one
