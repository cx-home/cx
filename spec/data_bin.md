...
canonical text form (see `spec/canonical.md` §3) but in compact bytes.
...
─────────────────────────────────────────────────────────────────
0 4 magic "CXDB" (0x43 0x58 0x44 0x42)
4 1 version 0x01 (CXDB v1)
5 1 flags bit 0 = endian (1 = LE; 0 reserved)
 bit 1 = schema-driven encoding
 (1 = schema reference precedes
 root value per §3.13; 0 = self-
 describing root value)
 bits 2-7 reserved (must be zero)
6 4 max_depth uint32 LE: maximum recursion depth allowed
...
Total fixed header: **12 bytes**. When flag bit 1 is unset, the root value
follows immediately at offset 12. When flag bit 1 is set, a schema
reference (§3.13.1) intervenes between offset 12 and the root value.
...
signals reader / writer support; the format-stability commitment (per
)
applies from v0.6.0 forward.
...
Per the host-type-mapping spec (`spec/type_mapping.md`), bindings whose
...
Bindings map to host decimal types per `spec/type_mapping.md`:
...
strict canonicalization (per `spec/canonical.md` §2.6).
...
Each pair: `<string-key>` (tag `0x30` mandatory) followed by `<value>`.
Pair order is preserved insertion order from the source. Sorting is not
performed.

**Duplicate key detection**: readers track keys within each map. On
detecting a duplicate key (per NFC-normalized comparison), readers error
unless the binding's `loads` was called with `strict=false` (in which case
last-wins and a warning is recorded). Writers never emit duplicate keys
in canonical form — they error on encountering duplicate keys in the
source AST.
...
Promoted from §5 reservations to a normative container at v0.6.0. A
chunked table declares its column schema once and emits zero or more
...
[`spec/abi.md §3`](abi.md). Design rationale and the canonical-origin
framing live in
...
```
43 58 44 42 01 01 40 00 00 00 00 00 header (12 bytes; CXDB v1, LE, no schema-driven)
...
```
43 58 44 42 01 01 40 00 00 00 00 00 header
63 chunked-table tag
02 col_count = 2
...
wrapper applies at the **row-group body** position; v0.6.0 does not
permit `0x90` at the value-level (root, map value, array element) or
inside a column payload.

Capability bit 22 (`0x400000`) signals reader / writer support per
[`spec/abi.md §3`](abi.md).
...
ints, repeated keys) zstd compresses CXDB row groups by 2-5×.
...
undeclared fields fall back to self-describing CXDB encoding (the
...
variant). Validation (per [`spec/schema.md`](schema.md)) runs
...
| tag | form | payload |
|------|------|---------|
| 0x10 | content-hash only | 32 bytes: SHA-256 of schema's CXDB strict-canonical encoding |
| 0x11 | inline schema | uvarint(schema_byte_len) <schema_byte_len bytes>; the schema as a recursive CXDB blob |
...
[`spec/canonical.md`](canonical.md) and for rationale.
...
1. **Key.** Encoded as a normal string (`0x30 uvarint(len) bytes`).
 Schema-driven encoding does not omit map keys in v0.6.0.
2. **Value.** If the schema declares an `[attr <key> :<T> ...]`
 ...
 key (under `open` mode per [`spec/schema.md §9`](schema.md)) the
 value is encoded with full tag (self-describing fallback).

For each scalar body (`[name :T value]`):

- If the schema declares `[body :<T> ...]`, the body's type tag is
 omitted; the value is encoded as the typed payload only.
- If `[body ...]` is `:elem` or `:mixed`, children encode normally
 (each child Element is a `0x50` map; recursion applies).
...
- Per-cell type bytes are already omitted by the §3.10 column-major
 ...
 column-internal data** in v0.6.0.
- Schemas that declare `[body :table[<col-spec>...]]` matching the
 table's column declarations enable validation but do not change
 the wire layout. Future minor revisions may extend tag-omission
 to drop the col-spec when schema declares it.

For arrays (`0x40`):

- v0.6.0 keeps array elements self-describing. Schemas declare
 `:T[]` (typed array) which validates the array but does not yet
 drive tag-omission for array elements; this is a candidate for
 v0.7.
...
```
43 58 44 42 01 01 40 00 00 00 00 00 header (flags = 0x01 = LE)
...
```
43 58 44 42 01 03 40 00 00 00 00 00 header (flags = 0x03 = LE | schema-driven)
...
emit time (`S012`); a schema-driven CXDB document with `region`
...
**Note (v0.6.0).** Per
 D11,
random-access footer indexing is **deferred indefinitely**. CXDB's
...
### 5.4 Run-length encoded columns (`0x73`)

For columns with long runs of repeated values. Compact wire form for
sorted or low-entropy data.

**Note (v0.6.0).** Also deferred indefinitely D11 — RLE
is one of the encoding-density features CXDB cedes to Parquet via
the Arrow bridge.

### 5.5 Promoted tags (v0.6.0)

Tags `0x63` (chunked table) and `0x90` (page-compression wrapper)
were listed as reserved in CXDB v1; they are **normative as of
v0.6.0** and specified in §3.11 / §3.12. v0.5.x readers reject
documents that use these tags; the format-stability boundary at
v0.6.0 means v0.6.0+ writers MAY emit them and v0.6.0+ readers MUST
accept them per the capability bits they advertise.
...
4. Rejects NaN, Inf, duplicate keys, invalid UTF-8, and integer overflow
 per `spec/canonical.md` validity preconditions.
5. Includes the file header in every output.
6. Sets reserved bits to zero.

**v0.6.0+ additional requirements:**
...
8. Errors when a value's bigint tag exceeds the host's representable
 range (per `spec/type_mapping.md`).
...
The conformance suite (see `spec/architecture.md` §Conformance) includes
binary fixture files: `fixture.input.cx` + expected `fixture.data.bin`.
All bindings produce byte-identical `data.bin` output for these inputs.
Drift is a conformance failure.

**v0.6.0+ fixture additions** (see `conformance/data_bin_chunked.txt`,
...
`CXDB v1` is the format version declared by the version byte (`0x01`).
...
```
43 58 44 42 01 01 40 00 00 00 00 00 header (12 bytes)
50 map tag
03 pair_count = 3
...
- **Random-access footer**: requires §5.3 (v2). Per,
 deferred indefinitely; CXDB transcodes to Parquet via the Arrow
 bridge for footer-indexed access.
- **String pool**: requires §5.2 (v2).
- **Schema-driven tag-omission for tables and arrays**: §3.13 v0.6.0
 ships per-attribute and per-scalar-body tag-omission; column-spec
 omission inside `0x60` / `0x62` / `0x63` containers and per-element
 tag-omission inside `0x40` arrays are candidates for v0.7.
- **Per-column compression**: §3.12 v0.6.0 ships row-group-level
 compression; per-column compression with codec selection is a
 v0.7 candidate.
