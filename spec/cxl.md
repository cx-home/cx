...
CXL is the CX Language — a CX-native expression language for
...
[`conformance/cxl.txt`](../conformance/cxl.txt).
...
### 5.3 `[?cx cxl-version=1]`

Pins the program to a specific CXL spec version. Evaluators with a
newer CXL MUST evaluate the program under the pinned version's
semantics. Future-version directives in a v1-pinned program are an
error.
...
[`conformance/cxl.txt`](../conformance/cxl.txt). v0.6.0 fixtures
