...
// and return unframed CXDB payload bytes; the dumper takes a framed
// CXDB buffer + optional schema hint and returns canonical CX text.
//
// refForm: 0 = content-hash only (default; spec/data_bin.md §3.13.1
...
func ToDataBinSchemaDriven(input, schema string, refForm int, nameHint string) ([]byte, error) {
	cInput := C.CString(input);  defer C.free(unsafe.Pointer(cInput))
	cSchema := C.CString(schema); defer C.free(unsafe.Pointer(cSchema))
	cHint := C.CString(nameHint); defer C.free(unsafe.Pointer(cHint))
	var errPtr *C.char
...
func MdToDataBinSchemaDriven(input, schema string, refForm int, nameHint string) ([]byte, error) {
	cInput := C.CString(input);  defer C.free(unsafe.Pointer(cInput))
	cSchema := C.CString(schema); defer C.free(unsafe.Pointer(cSchema))
	cHint := C.CString(nameHint); defer C.free(unsafe.Pointer(cHint))
	var errPtr *C.char
	raw := unsafe.Pointer(C.cx_md_to_data_bin_schema_driven(cInput, cSchema, C.int(refForm), cHint, &errPtr))
	return extractBinPayload(raw, errPtr)
}
...
// FromDataBinSchemaDriven decodes a framed CXDB schema-driven buffer to
...
func FromDataBinSchemaDriven(framed []byte, schemaHint string) (string, error) {
	...
	cHint := C.CString(schemaHint); defer C.free(unsafe.Pointer(cHint))
	var errPtr *C.char
