...
pub mut:
	...
	// is_ref = false. Round-trip preserves the bare form.
	is_ref    bool
...
pub mut:
	...
	attrs     []Attr
	items     []Node
	// v3.4 (ADR 0003): syntactic ID declaration ("#name" token); none when
	// the element has no ID. Distinct from anchor.
	id        ?string
...
fn ast_node_from_any(v json2.Any) !Node {
	...
	return match typ {
		...
		'Interpolation'   { Node(InterpolationNode{ expr: jobj_str(obj, 'expr') }) }
		'EvalDirective'   { Node(ast_eval_directive_from_any(obj)!) }
...
fn ast_element_from_any(obj map[string]json2.Any) !Element {
	...
	return Element{
		...
		attrs:     attrs
		items:     items
		id:        jobj_str_opt(obj, 'id')
		body_ref:  jobj_str_opt(obj, 'bodyRef')
	}
}

fn ast_attr_from_any(obj map[string]json2.Any) !Attr {
	name := jobj_str(obj, 'name')
	is_ref := if v := obj['isRef'] { v.bool() } else { false }
	// v3.5 (ADR 0016): BracketBody attribute body.
	if body_any := obj['body'] {
		mut body := []Node{}
		for n in body_any.as_array() {
			body << ast_node_from_any(n)!
		}
		return Attr{
			name:      name
			value:     ScalarVal('')
...
fn ast_eval_directive_from_any(obj map[string]json2.Any) !EvalDirectiveNode {
	name := jobj_str(obj, 'name')
	mut attrs := []Attr{}
	for a in jobj_arr(obj, 'attrs') {
		attrs << ast_attr_from_any(a.as_map())!
	...
	return EvalDirectiveNode{ name: name, attrs: attrs, items: items }
}
...
fn ast_emit_node(n Node, depth int, mut out []string) {
	match n {
		...
		EvalDirectiveNode {
			...
			for item in n.items {
				mut tmp := []string{}
				ast_emit_node(item, 0, mut tmp)
				body_parts << tmp.join('').trim_right('\n')
			}
			body := body_parts.join(' ')
			sep := if body.len > 0 { ' ' } else { '' }
			out << '${cx_ind(depth)}[?${n.name}${attrs}${sep}${body}]\n'
		}
	}
}
