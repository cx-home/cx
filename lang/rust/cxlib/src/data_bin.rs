...
extern "C" {
    ...
    fn cx_md_to_data_bin  (input: *const c_char, err_out: *mut *mut c_char) -> *mut c_char;
    ...
    fn cx_data_bin_to_md  (input: *const c_char, err_out: *mut *mut c_char) -> *mut c_char;
...
/// Encode CX text to a CXDB v1 framed buffer via the C ABI.
...
/// Decode a CXDB v1 framed buffer to canonical CX text via the C ABI.
...
pub fn decode_payload(payload: &[u8]) -> Result<Value, String> {
    if payload.len() < 12 {
        return Err("cxdb: payload too short for 12-byte header".to_owned());
    }
    if &payload[0..4] != CXDB_MAGIC {
        return Err("cxdb: bad magic (expected 'CXDB')".to_owned());
    }
    if payload[4] != CXDB_VERSION {
        return Err(format!("cxdb: unsupported version {}", payload[4]));
    }
    let flags = payload[5];
    if flags & 0xFE != 0 {
        return Err("cxdb: reserved flag bits set".to_owned());
    }
    if flags & 0x01 == 0 {
        return Err("cxdb: only little-endian payloads supported in v1".to_owned());
    }
    if payload[10] != 0 || payload[11] != 0 {
        return Err("cxdb: reserved header bytes must be zero".to_owned());
    }
    let mut r = Reader::new(&payload[12..]);
    r.value()
}

/// Encode a serde_json::Value as a framed CXDB v1 buffer.
pub fn encode(value: &Value) -> Result<Vec<u8>, String> {
    let mut w = Writer::new();
    // Header
    w.buf.extend_from_slice(CXDB_MAGIC);
    w.buf.push(CXDB_VERSION);
    w.buf.push(CXDB_FLAGS_LE);
...
impl<'a> Reader<'a> {
    fn new(buf: &'a [u8]) -> Self {
        Self { buf, pos: 0, depth: 0, max_depth: CXDB_DEFAULT_DEPTH as usize }
    }

    fn take(&mut self, n: usize) -> Result<&'a [u8], String> {
        if self.pos + n > self.buf.len() {
            return Err(format!("cxdb: {} bytes requested, {} remaining", n, self.buf.len() - self.pos));
        }
    ...
    fn u8(&mut self) -> Result<u8, String> {
        if self.pos >= self.buf.len() {
            return Err("cxdb: unexpected end of input".to_owned());
        }
    ...
    fn uvarint(&mut self) -> Result<u64, String> {
        ...
        for i in 0..5 {
            ...
            if b < 0x80 {
                if i == 4 && b > 0x0F {
                    return Err("cxdb: varint overflow".to_owned());
                }
                if i > 0 && b == 0 {
                    return Err("cxdb: non-canonical varint (extra zero byte)".to_owned());
                }
        ...
        Err("cxdb: varint exceeds 5 bytes".to_owned())
    }
    ...
    fn value(&mut self) -> Result<Value, String> {
        ...
        if self.depth > self.max_depth {
            return Err(format!("cxdb: recursion depth exceeds limit ({})", self.max_depth));
        }
        let tag = self.u8()?;
        let result = match tag {
            ...
            TAG_FLOAT64 => {
                ...
                Number::from_f64(v).map(Value::Number).ok_or_else(|| "cxdb: NaN/Inf rejected".to_owned())
            }
            ...
            TAG_ARRAY => {
                ...
                if count == 0 {
                    return Err("cxdb: array tag 0x40 with count=0; use 0x41 for empty".to_owned());
                }
            ...
            TAG_MAP => {
                let count = self.uvarint()? as usize;
                if count == 0 {
                    return Err("cxdb: map tag 0x50 with count=0; use 0x51 for empty".to_owned());
                }
                let mut out = Map::with_capacity(count);
                for _ in 0..count {
                    let key_tag = self.u8()?;
                    if key_tag != TAG_STRING {
                        return Err(format!("cxdb: map key must be string; got 0x{:02x}", key_tag));
                    }
            ...
            other => Err(format!("cxdb: unknown tag 0x{:02x} at offset {}", other, self.pos - 1)),
        };
        self.depth -= 1;
        result
    }
    ...
    fn table_payload(&mut self, tag: u8) -> Result<Value, String> {
        ...
        for _ in 0..col_count {
            let key_tag = self.u8()?;
            if key_tag != TAG_STRING {
                return Err(format!("cxdb: table column name must be string; got 0x{:02x}", key_tag));
            }
            ...
            Value::Number(n) => {
                if let Some(i) = n.as_i64() {
                    ...
                    if u <= i64::MAX as u64 {
                        ...
                        return Err(format!("cxdb: u64 {} exceeds i64 range", u));
                    }
                } else if let Some(f) = n.as_f64() {
                    self.u8(TAG_FLOAT64);
                    self.f64(f);
                } else {
                    return Err("cxdb: unsupported Number variant".to_owned());
                }
            }
